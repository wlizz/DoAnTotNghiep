...
export default function ChatBox() {
  ...
  const normalize = (str: string) => {
    return str
      ...
      // chuẩn hóa từ khóa
      .replace(/k/g, "000")
      .replace(/<|duoi|nho hon/g, "duoi")
      .replace(/>|tren|lon hon/g, "tren")
      .replace(/gia/g, "duoi")
      // bỏ ký tự thừa
      .replace(/[^\w\s]/g, "")
      .trim();
  ...
  const getFavoriteType = () => {
    ...
    for (let key in data) {
      // ❌ bỏ key không phải loại sản phẩm
      if (key === "favoritePrice" || key === "lastVisit") continue;
  ...
  const handleBotResponse = async (text: string): Promise<Message> => {
    const lower = normalize(text);
    if (
      lower.includes("goi y rieng") ||
      lower.includes("danh cho toi") ||
      lower.includes("recommend cho toi")
    ) {
      try {
        ...
        if (!fav) {
          return {
            type: "bot",
            text: "🤖 Bạn chưa có lịch sử, hãy thử xem sản phẩm để mình gợi ý nhé!",
          };
        }
        const res = await api.product.getAllProduct(1, 100, {
          name: "",
    ...
    if (
      lower.includes("goi y") ||
      lower.includes("recommend") ||
      lower.includes("nen mua") ||
